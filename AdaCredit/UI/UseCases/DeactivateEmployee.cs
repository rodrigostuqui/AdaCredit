...
namespace AdaCredit.UI.UseCases
{
    public static class DeactivateEmployee
    {
        public static void Run()
        {
            ...
            deactivate.Clicked += () =>
            {
                var numberError = EmployeeServices.ChangeEmployeeStatus((string)cpfText.Text, false);
                if (numberError == 0)
                {
            ...
            activate.Clicked += () =>
            {
                var numberError = EmployeeServices.ChangeEmployeeStatus((string)cpfText.Text, true);
                if (numberError == 0)
                {
                    MessageBox.Query("Cadastro Atualizado", "Status atualizado com sucesso", "Ok");
                    Application.Shutdown();
